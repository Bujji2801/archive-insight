...
// src/scripts/generateProjects.ts
...
function generateProjects(count = 54) {
  const projects = [];
  for (let i = 0; i < count; i++) {
    const year = getRandomItem(YEARS);
    const branch = getRandomItem(DEPARTMENTS);
    const techStack = getRandomItems(TECH_STACKS, Math.floor(Math.random() * 3) + 2);
    ...
    const project = {
      ...
      userId: `user-${Math.floor(Math.random() * 1e3)}`
    };
...
var supabaseUrl = "https://ikwytehkerkeavqfdilc.supabase.co";
var supabaseKey = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imlrd3l0ZWhrZXJrZWF2cWZkaWxjIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzA3NTk4ODYsImV4cCI6MjA4NjMzNTg4Nn0.heRpgfAO0cTbA1209pHt9nJyrGWS8jGiQJcxyUTCbFs";
if (!supabaseUrl || !supabaseKey) {
  throw new Error("Missing Supabase environment variables in .env file");
}
var supabase = createClient(supabaseUrl, supabaseKey);
var seedProjects = async () => {
  console.log("\u{1F331} Generating and Seeding projects...");
...
seedProjects().then(() => console.log("Done.")).catch((err) => console.error("Fatal:", err));
